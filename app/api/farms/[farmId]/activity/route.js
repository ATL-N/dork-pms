...
const activityMessageMap = {
    'VACCINATION_RECORDED': (log) => `Vaccination '${log.meta?.vaccine}' recorded for ${log.meta?.flockName}.`,
    'MORTALITY_RECORDED': (log) => `${log.meta?.quantity} mortalities recorded for ${log.meta?.flockName}.`,
    'FEED_CONSUMPTION_RECORDED': (log) => `Feed consumption recorded for ${log.meta?.flockName}.`,
    'EXPENSE_ADDED': (log) => `New expense of ${log.meta?.amount} added for ${log.meta?.category}.`,
    'REVENUE_ADDED': (log) => `New revenue of ${log.meta?.amount} from ${log.meta?.category}.`,
    'FLOCK_CREATED': (log) => `New flock '${log.meta?.flockName}' was created.`,
    'USER_INVITED': (log) => `${log.meta?.email} was invited to the farm as a ${log.meta?.role}.`,
};
...
export async function GET(req, context) {
  try {
    ...
    const formattedActivities = logs.map(log => {
        const messageGenerator = activityMessageMap[log.message] || (() => log.message);
        return {
            id: log.id,
