...
export async function POST(request, { params }) {
    ...
    try {
        ...
        if (!farmUser) {
            return NextResponse.json({ error: 'You do not have permission to add flocks to this farm.' }, { status: 403 });
        }

        const data = await request.json();
        const { name, type, breed, quantity, startDate, location, costPerBird } = data;
        ...
        await logAction({
            level: "ERROR",
            message: `Failed to create flock for farm ${farmId}. Error: ${error.message}`,
            userId: user.id,
            meta: { farmId, stack: error.stack },
        });
